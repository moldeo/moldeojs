...
export class moEffectSound extends MO.moEffect {
  ...
  sound_samples: any = "";//check mp3s and wavs
  soundLoaded : any = false;

  sound_reverb : any = false;
  soundReverb : any = false;
  sound_delay : any = false;
  soundDelay : any = false;
  sound_rate : any = false;
    ...
    */
    ...
    if (this.PreInit((res) => {
      ...
      this.p5.prototype.userStartAudio( this.elstart, function() {
        ...
        let soundsample = self.m_Config.Text("sound_samples")
        if (self.sound_samples!=soundsample) {
          self.sound_samples = soundsample;
          console.log(self.sound_samples)
          self.soundLoaded = self.p5.prototype.loadSound("assets/molrepos/moldeoorg/fabri/Musas/"+soundsample, (ev) => {
            console.log("Loaded! ")
            console.log(ev)
            console.log(this)
            console.log(self)
            //self.soundLoaded.play();
            self.soundLoaded.loop();
          });
          source = self.soundLoaded;
          //self.soundLoaded.setLoop(true);
          //self.soundLoaded.play();
          //setTimeout( () => {
          //    console.log()
          //}
          console.log(self.soundLoaded)
        }

        self.sound_rate = self.m_Config.Eval("sound_rate");
        ...
        if (self.processor) source = self.processor;

        self.sound_delay = self.m_Config.Eval("sound_delay");
        if (self.sound_delay && false) {
          console.log("sound_delay found not null");
          console.log(self.sound_delay);
          let feedback = 0.7;
          let filterFrequency = 2300;
          console.log("Create new soundDelay");
          console.log(self.soundDelay);
          self.soundDelay = new self.p5.Delay();
          console.log(self.soundDelay);
          console.log("source:")
          console.log(source)
          self.soundDelay.process( source, self.sound_delay, feedback, filterFrequency)
          console.log(self.soundDelay);
        }

        self.sound_reverb = self.m_Config.Eval("sound_reverb");
        if (self.sound_reverb) {
          console.log("sound_reverb");
          console.log(self.sound_reverb);
        }
  ...
  UpdateParameters() : void {
    ...
    this.sound_rate = this.m_Config.Eval("sound_rate");

    let sound_delay = this.m_Config.Eval("sound_delay");
    let sound_delay_freq = this.m_Config.Eval("sound_delay_freq");
    if (this.sound_delay!=sound_delay && this.soundDelay) {
      this.sound_delay = sound_delay;
      console.log("sound_delay");
      console.log(this.sound_delay);
      let feedback = 0.7;
      let filterFrequency = 2300;
      if (sound_delay_freq) {
        filterFrequency = sound_delay_freq;
      }
      this.soundDelay.delayTime(this.sound_delay);
      this.soundDelay.filter(filterFrequency);
      //self.soundDelay = new self.p5.Delay();
      //self.soundDelay.process( source, self.sound_delay, feedback, filterFrequency)
    }
    ...
    if (this.soundLoaded) {
      this.soundLoaded.setVolume(this.sound_volume);
      //this.sound_rate = this.p5.constrain(this.sound_rate*4.0, 0.01, 4);
      //console.log(this.sound_rate)
      this.soundLoaded.rate(this.sound_rate);
    }
  }
  ...
  audioon() :void {
    if (this.playing) {
      ...
      if (this.soundLoaded) { this.soundLoaded.loop(); }
      this.playing = true;
  ...
  audiooff() :void {
    ...
    if (this.playing) {
      ...
      if (this.soundLoaded) { this.soundLoaded.stop(); }
      this.playing = false;
